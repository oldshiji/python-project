...
class Model(object):
    ...
    def save(self,data)->'保存数据':
        ...
        sql = "INSERT INTO {0}({1}) VALUES('{2}')".format(self.tableName,",".join(field),"','".join(values))
        self.connect()
        try:
            self.cursor.execute(sql)
    ...
    def get(self)->'获取表所有记录':
        if self.tableName==None:
            print("数据表未设置")
            return 0
        if len(self.limitOffset) > 2 and len(self.orderby) > 0:

            sql = "SELECT %s FROM %s ORDER BY %s LIMIT %s " % (
            self.fieldName, self.tableName, self.orderby, self.limitOffset)
        elif len(self.orderby)>0:
    ...
    def limit(self,limit,offset):
        ...
        self.limitOffset = "%d,%d"%(limit,offset)
        return self
    def count(self):

        sql = "SELECT COUNT(*) AS p FROM %s"%(self.tableName)
        self.connect()
        try:
            self.cursor.execute(sql)
            self.connection.commit()
