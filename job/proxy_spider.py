...
#61.132.93.14:6460:16VJHCMU:599315
# 代理服务器
...
# 访问三次网站，使用相同的`:::HTML Proxy-Authorization`标志，均能够保持相同的外网IP
'''
for i in range(3):
    s = requests.session()
...
def yiniu_proxy(url,cookie,type,data):
    headers = {
    ...
    if type=='get':
        ...
        cookie = r.cookies
        text = r.text
    elif type=='get' and cookie:
        r = s.get(url, proxies=proxies, headers=headers,cookies=cookie)
        cookie = r.cookies
        text = r.text
    elif type=='post':
        r = s.post(url, proxies=proxies, headers=headers, data=data)
        cookie = r.cookies
        text = r.text
    elif type=='post' and cookie:
        r = s.post(url, proxies=proxies, headers=headers, data=data,cookies=cookie)
        cookie = r.cookies
        text = r.text
