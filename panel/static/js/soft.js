			...
			if(rdata.data[i].name != 'php'){
				...
				if(rdata.data[i].price > 0){
					...
					if(rdata.data[i].end == '未开通' || rdata.data[i].end == '已到期' || rdata.data[i].end == '待支付'){
						handle = '<a class="btlink" onclick="Renewinstall(\''+rdata.data[i].title+'\',\''+rdata.data[i].product_id+'\')">立即购买</a>'
						titleClick = 'onclick="Renewinstall(\''+rdata.data[i].title+'\',\''+rdata.data[i].product_id+'\')" style="cursor:pointer"';
						if(rdata.data[i].end == '已到期') handle = '<a class="btlink" onclick="Renewinstall(\''+rdata.data[i].title+'\',\''+rdata.data[i].product_id+'\',1)">立即续费</a>'
	...
	var payhtml = '<div class="libPay" style="padding:15px 30px 30px 30px">\
				<div class="libPay-item f14 plr15 libPay-select">\
					...
					<div class="li-con c6">\
						...
						<p class="pro-info" style="position:absolute;top:151px;left:42px;color: #FF7301;font-size: 12px;display:none">（专业版过期了将需要扫码支付后才能登陆或者在SSH执行免费版升级命令来切换成免费版）</p>\
					</div>\
				</div>\
